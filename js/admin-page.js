      ...
      return `<div class="apanel">
        ...
        return `<div class="arow">
            <div class="arow-main"><img class="acd-img" src="${ava(memberName(n.usn))}" alt="">
              ...
              <p class="amuted">diusulkan @${esc(n.by)}${n.note ? ` — “${esc(n.note)}”` : ''}</p></div></div>
            <div class="arow-acts">
  ...
  body.innerHTML = `
    ...
    <div class="apanel">
      <div class="apanel-head"><div><h3>Segel Pemenang</h3>
        <p class="amuted">🥁 Klik untuk drum roll 4 detik, lalu segel terbuka. Buka satu-satu biar dramatis.</p></div></div>
      ${PUB.categories.map(c => {
    const isRev = !!(PUB.revealed && PUB.revealed[c.id]);
    const w = isRev ? (PUB.winners || {})[c.id] : null;
    return `<div class="arow">
          <div><b>${esc(c.name)}</b>${(PUB.secret || {})[c.id] ? ' <span class="badge badge-pend">🔒 RAHASIA</span>' : ''}
            <p class="amuted">${isRev && w ? 'Pemenang: ' + esc(memberName(w.usn)) + (w.unanimous ? ' (ditetapkan panitia)' : ' (' + w.votes + ' suara)') : 'Masih tersegel'}</p></div>
          <div class="arow-acts">
      ...
      <div class="btnrow" style="margin-top:16px">
        <button type="button" class="btn btn-gold sm" data-act="revealAll">🥁 Buka Semua (satu per satu)</button>
...
function initAdminPage() {
  ...
  firebase.auth().onAuthStateChanged(user => {
    ...
    if (adminOk) {
      Cloud.listenAdmin();
      Cloud.onChange(() => {
        if (aTab === 'kontrol' && document.activeElement?.tagName === 'INPUT') return;
        renderTab();
      });
  ...
  document.getElementById('aBody').addEventListener('click', e => {
    ...
    if (act === 'addCat') {
    ...
    else if (act === 'revealCat') {
      DrumRoll.roll(null, 3500);              // admin ikut dengar drum
      el.disabled = true; el.textContent = '🥁 4 detik…';
      Cloud.startPre(id);
      setTimeout(() => {
        Cloud.stopPre(id);
        Cloud.revealCat(id);
      ...
      if (!confirm('Buka segel SEMUA kategori satu per satu (dengan drum roll)?')) return;
      const cats = [...PUB.categories];
      const seq = (i) => {
        if (i >= cats.length) { Confetti.burst(180); toast('Semua segel dibuka ✦', 'Selamat kepada para pemenang!'); return; }
        Cloud.startPre(cats[i].id);
        setTimeout(() => {
          Cloud.stopPre(cats[i].id); Cloud.revealCat(cats[i].id);
          setTimeout(() => seq(i + 1), 900);
        }, 4000);
      };
      seq(0);
    }
